      ...
      <div class="news-model-cont">
          <div class="news-model-left">
              <div class="title-symbol">
                  <div>{{helpsdata.title}}</div>
                  <div></div>
                  <div>{{helpsdata.englishTitle}}</div>
              </div>
          </div>
          <div class="news-model-right">
              <router-link :to="'/newsDetails/' + item.id" v-for="item in helpsdata.list" :key="item.id" class="news-item-box">
   ...
   methods:{
    ...
    handleSizeChange(val) {
      ...
      ajaxdata:function(pagenum){
        let that=this;
        //请求新闻news
        let datanews={type_id:'4',page:pagenum,num:'10'}
        this.$axios({
          method: 'post',
          ...
          data: qs.stringify(datanews) 
            ...
            if(response.data.status=="ok"){
              console.log("news")
              that.helpsdata.list=response.data.data.data
